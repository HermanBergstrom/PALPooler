...
When no validation split is present the last iteration is used instead.
...
def _best_or_last(stages: list[dict], acc_key: str) -> tuple[Optional[float], Optional[float]]:
    """Return (final_acc, best_val_acc) for one seed's stages."""
    iters = [s for s in stages if s.get("tag", "").startswith("iter_")]
    if not iters:
        return None, None
    final = iters[-1].get(acc_key)
    val_iters = [s for s in iters if s.get("val_accuracy") is not None]
    if val_iters:
        best = max(val_iters, key=lambda s: s["val_accuracy"])
        best_val = best.get(acc_key)
    else:
        best_val = final  # no val split — last IS the selection
    return final, best_val


def _load_variant(exp_dir: Path, metric: str) -> Optional[dict]:
    """Aggregate stats across all seed_* subdirs; return None if nothing found."""
    ...
    if not seed_dirs:
        ...
        seed_dirs = [exp_dir] if rj.exists() else []

    finals, best_vals, cls_vals, mp_vals = [], [], [], []
    for sd in seed_dirs:
        rj = sd / "results.json" if (sd / "results.json").exists() else sd.parent / "results.json"
        if not rj.exists():
            continue
        data = _load(rj)
        ...
        final, best_val = _best_or_last(stages, acc_key)
        finals.append(final)
...
def load_dataset_data(results_dir: Path, dataset: str, metric: str) -> Optional[list[dict]]:
    """Return list of group dicts (one per GROUPS entry); None if nothing found."""
    groups_out = []
    for group_label, variants in GROUPS:
        ...
        for suffix, bar_label, color, hatch in variants:
            exp_dir = results_dir / f"{dataset}_{suffix}"
            vdata = _load_variant(exp_dir, metric)
            if vdata is None:
                continue
            if not baselines:
                baselines = {
...
def _plot_dataset(ax: plt.Axes, groups: list[dict], dataset: str, metric: str) -> None:
    ...
    for group in groups:
        ...
        for vd in group["variants"]:
            ...
            m, s  = vd["final"]
            bv, _ = vd["best_val"]
            bar_entries.append((x, vd["label"], m * scale, (s or 0.0) * scale,
                                vd["color"], vd["hatch"],
    ...
    for gx, glabel in group_label_positions:
        ax.text(gx, label_y, glabel, ha="center", va="top",
                fontsize=8, color="#555555", style="italic", clip_on=False)
...
def plot_img_pal_pooling(
    ...
    if output_path is None:
        ...
        output_path = results_dir / f"img_pal_pooling_plot{suf}.png"
...
def _parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Plot image PAL pooling results")
